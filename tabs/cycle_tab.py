...
def _is_tank_cycled(df: pd.DataFrame) -> bool:
    ...
    Args:
        df: A Pandas DataFrame containing water test history for a single tank,
            expected to be sorted by date in ascending order.
            Must include 'ammonia', 'nitrite', and 'nitrate' columns.

    Returns:
        True if the tank meets the criteria for being cycled, False otherwise.
    """
...
def cycle_tab(key_prefix: str = ""):
    """
    Renders the "Nitrogen Cycle Tracker" tab in the Streamlit application.

    This tab displays the historical trends of ammonia, nitrite, and nitrate
    for the selected tank, assesses whether the tank is cycled, and provides
    guidance on interpreting the nitrogen cycle.
