...
def _load_failed_tests(tank_id: int | None) -> _pd.DataFrame:
    ...
    Args:
        tank_id: The ID of the tank to load failed tests for. If None, behavior
                 might vary based on `fetch_by_date_range` implementation, but
                 typically a tank_id is expected for this context.
    ...
    # 2. Instantiate the repository and call its method to fetch all data for the tank.
...
def failed_tests_tab() -> None:
    ...
    This tab displays a historical overview of all water test records for the
    currently selected tank where one or more parameters were outside their
    configured safe ranges. It provides a tabular view with visual highlighting
    ...
    # 3. Instantiate the TankRepository to get the tank's friendly name.
    ...
    # Localize and rename headers for display.
    rename_map = {c: translate(c.capitalize()) for c in display_df.columns}
    # Specific override for GH display label if needed.
    if "gh" in rename_map:
        rename_map["gh"] = "GH (°dH)"
    if "kh" in rename_map:
    ...
    # Apply conditional highlighting to out-of-range cells in the DataFrame.
    styled = highlight_out_of_range(display_df, SAFE_RANGES)

    # Display the styled table.
    # On mobile, wrap in an expander for better screen real estate management.
    if is_mobile():
