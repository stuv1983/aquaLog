...
def _ensure_owned_fish_schema() -> None:
    ...
    if 'tank_id' not in cols:
        cur.execute("ALTER TABLE owned_fish ADD COLUMN tank_id INTEGER DEFAULT 1;")
        conn.commit()
    conn.close()
...
def fish_inventory_tab() -> None:
    ...
    with get_connection() as conn:
        ...
        # Normalize name columns: map CSV schema to our common fields
    if 'common_name' not in master_df.columns or master_df['common_name'].eq('').all():
        if 'name_english' in master_df.columns:
            master_df['common_name'] = master_df['name_english']
        else:
            master_df['common_name'] = ''
    if 'scientific_name' not in master_df.columns or master_df['scientific_name'].eq('').all():
        if 'name_latin' in master_df.columns:
            master_df['scientific_name'] = master_df['name_latin']
        else:
            master_df['scientific_name'] = ''

    # Remove raw name_english/name_latin to simplify searches
    drop_cols = [c for c in ('name_english','name_latin') if c in master_df.columns]
        ...
        if search_df.empty:
            ...
            for _, row in search_df.iterrows():
                ...
                with col1:
                    ...
                    img = img_url if img_url else f'https://selectyourfish.com/webps1/{fid}.webp'
    ...
    # Owned search
    term2 = st.text_input(
        '🔍 Search your owned fish',
        key='search_owned_fish'
    ).strip().lower()
    if term2:
        mask2 = owned.drop(columns=['fish_id','common_name']).apply(
            lambda r: any(term2 in str(v).lower() for v in r.values), axis=1
        )
        owned = owned[mask2]
        if owned.empty:
    ...
    for _, row in owned.iterrows():
        ...
        with c2:
            ...
            for field in [
                ...
                if pd.notna(val) and val != '':
                    lbl = field.replace('_',' ').title()
                    display = swim_map[val] if field=='swim' else val
