...
class CustomRangeRepository(BaseRepository):
    ...
    # Define a set of valid parameters for which custom ranges can be set.
    VALID_PARAMETERS = {
    ...
    def get(self, tank_id: int, parameter: str) -> Optional[Tuple[float, float]]:
        ...
        Args:
            tank_id: The unique identifier of the tank.
            parameter: The name of the parameter (e.g., "ph", "ammonia").

        Returns:
            A tuple (safe_low, safe_high) if a custom range is found,
            otherwise None.

        Raises:
            ValueError: If `tank_id` is invalid or `parameter` is not in `VALID_PARAMETERS`.
    ...
    def set(
        ...
        If a range for the given tank and parameter already exists, it will be updated.

        Args:
            tank_id: The unique identifier of the tank.
            parameter: The name of the parameter.
            low: The new safe low value for the parameter.
            high: The new safe high value for the parameter.

        Returns:
            A dictionary representing the inserted or updated custom range record.

        Raises:
            ValueError: If input validation fails (e.g., invalid tank ID, parameter,
                        or if high <= low).
    ...
    def get_all_for_tank(self, tank_id: int) -> Dict[str, Tuple[float, float]]:
        ...
        Returns:
            ...
            tuples (safe_low, safe_high) for that parameter.
    ...
    def _validate_tank_id(self, tank_id: int):
        """
        Validates if the provided `tank_id` is a positive integer.
    ...
    def _validate_parameter(self, parameter: str):
        """
        Validates if the provided `parameter` name is one of the allowed parameters.

        Args:
            parameter: The parameter name to validate.

        Raises:
            ValueError: If `parameter` is not in `VALID_PARAMETERS`.
        """
    ...
    def _validate_range_values(self, low: float, high: float):
        ...
        Raises:
            ValueError: If values are not numeric or if `high` is not greater than `low`.
        """
